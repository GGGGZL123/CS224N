...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) >0:
        ...
        #句子解析结束的条件：缓存清0且栈中只剩ROOT
        unfinished_parses = [t for t in unfinished_parses if len(t.buffer) > 0 or len(t.stack) > 1]
    dependencies = [t.dependencies for t in partial_parses]
    ### END YOUR CODE
